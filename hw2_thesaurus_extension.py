...
def augment_using_thesaurus(doc):
          author_synonyms = []
          for word in doc.author:
              for syn in wordnet.synsets(word):
                for lm in syn.lemmas():
                  author_synonyms.append(lm.name())
            
          title_synonyms = []
          for word in doc.title:
              for syn in wordnet.synsets(word):
                for lm in syn.lemmas():
                  title_synonyms.append(lm.name()) 
          
          keyword_synonyms = []
          for word in doc.keyword:
              for syn in wordnet.synsets(word):
                for lm in syn.lemmas():
                  keyword_synonyms.append(lm.name()) 
          
          abstract_synonyms = []
          for word in doc.abstract:
              for syn in wordnet.synsets(word):
                for lm in syn.lemmas():
                  abstract_synonyms.append(lm.name()) 
        
          return Document(doc.doc_id, 
                          doc.author + author_synonyms,
                          doc.title + title_synonyms,
                          doc.keyword + keyword_synonyms,
                          doc.abstract + abstract_synonyms)
...
def experiment():
    ...
    for term, stem, removestop, sim, term_weights in itertools.product(*permutations):
        ...
        doc_vectors = [term_funcs[term](augment_using_thesaurus(doc), doc_freqs, term_weights) for doc in processed_docs]

        metrics = []

        for query in processed_queries:
            augment_using_thesaurus(query)
